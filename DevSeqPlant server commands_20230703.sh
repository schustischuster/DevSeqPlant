...
# Correct csv import errors:
UPDATE `Arabidopsis_thaliana_gene_tpm_20230625`
SET `gene_id` = TRIM(BOTH '"' FROM `gene_id`);

UPDATE `Arabidopsis_thaliana_gene_tpm_20230625`
SET `symbol` = TRIM(BOTH '"' FROM `symbol`);

UPDATE `Arabidopsis_thaliana_gene_tpm_RE_20230625`
SET `gene_id` = TRIM(BOTH '"' FROM `gene_id`);

UPDATE `Arabidopsis_thaliana_gene_tpm_RE_20230625`
SET `symbol` = TRIM(BOTH '"' FROM `symbol`);

UPDATE `Arabidopsis_thaliana_transcript_tpm_20230625`
SET `transcript_id` = TRIM(BOTH '"' FROM `transcript_id`);
...
UPDATE `Arabidopsis_thaliana_transcript_tpm_RE_20230625`
SET `transcript_id` = TRIM(BOTH '"' FROM `transcript_id`);
...
UPDATE `Arabidopsis_thaliana_repl_gene_tpm_20230627`
SET `gene_id` = TRIM(BOTH '"' FROM `gene_id`);

UPDATE `Arabidopsis_thaliana_repl_gene_tpm_20230627`
SET `symbol` = TRIM(BOTH '"' FROM `symbol`);

UPDATE `Arabidopsis_thaliana_repl_gene_tpm_RE_20230627`
SET `gene_id` = TRIM(BOTH '"' FROM `gene_id`);

UPDATE `Arabidopsis_thaliana_repl_gene_tpm_RE_20230627`
SET `symbol` = TRIM(BOTH '"' FROM `symbol`);

UPDATE `Arabidopsis_thaliana_repl_transcript_tpm_20230627`
SET `transcript_id` = TRIM(BOTH '"' FROM `transcript_id`);
...
UPDATE `Arabidopsis_thaliana_repl_transcript_tpm_RE_20230627`
SET `transcript_id` = TRIM(BOTH '"' FROM `transcript_id`);
...
# -------------------------------------- #
# Upload/Update php index file on server
# -------------------------------------- #

# On local machine
# Set path to index file
cd /Users/hobbit/Documents/Christoph/DevSeqPlant_code_20220622

# Transfer php index file to server
scp testindex.php ubuntu@128.232.226.206:/var/www/devseqplant.org/

# Access and change config files on server
# Show all available websites
cd /etc/apache2/sites-available/

# Access config file, e.g. for devseqplant (http config)
sudo nano /etc/apache2/sites-available/devseqplant.org.conf


# ------------------------------- #
# Get and update ssl certificates
# ------------------------------- #
