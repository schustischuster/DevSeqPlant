# Prepare_AL_data_for_webpage - EvoSeq transcript expression data version from 20210528
...
devseq_transcripts_input_file <- "AL_transcripts_inter_norm_tpm_mat_deseq_sample_names.csv"
devseq_transcripts_input_file_pol <- "AL_transcripts_intra_norm_tpm_mat_deseq_sample_names.csv"


# Read raw data
devseq_transcripts_all_samples <- read.table(file=file.path(in_dir, "AL", devseq_transcripts_input_file), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
devseq_transcripts_all_samples_pol <- read.table(file=file.path(in_dir, "AL", devseq_transcripts_input_file_pol), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
...
devseq_transcripts_all_samples_pol <- devseq_transcripts_all_samples_pol[c("id", 
	"flowers_mature_pollen_8w.10w.25d_.1.", "flowers_mature_pollen_8w.10w.25d_.2.","flowers_mature_pollen_8w.10w.25d_.3.")]
...
devseq_transcripts_all_samples = devseq_transcripts_all_samples %>% select(
			transcript_id, 
			root_whole_root_6d_.1.:flower_stg12_8w.10w.15d_.3.,
			...
			flower_mid_stg12_stamens_8w.10w.24d_.1.,
			flower_mid_stg12_stamens_8w.10w.24d_.2.,
			flower_mid_stg12_stamens_8w.10w.24d_.3.,
			flower_late_stg12_stamens_8w.10w.21d_.1.,
			flower_late_stg12_stamens_8w.10w.21d_.2.,
			flower_late_stg12_stamens_8w.10w.21d_.3.,
			flowers_mature_pollen_8w.10w.25d_.1.,
			flowers_mature_pollen_8w.10w.25d_.2.,
			flowers_mature_pollen_8w.10w.25d_.3.,
			flower_early_stg12_carpels_8w.10w.17d_.1.,
			flower_early_stg12_carpels_8w.10w.17d_.2.,
			flower_early_stg12_carpels_8w.10w.17d_.3.
			)
...
write.table(devseq_transcripts_all_samples, file=file.path(out_dir, "output", "data_tables", "AL_devseq_transcripts_all_samples.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_transcripts_all_samples_RE, file=file.path(out_dir, "output", "data_tables", "AL_devseq_transcripts_all_samples_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts, file=file.path(out_dir, "output", "data_tables", "AL_devseq_replicate_transcripts.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts_RE, file=file.path(out_dir, "output", "data_tables", "AL_devseq_replicate_transcripts_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
