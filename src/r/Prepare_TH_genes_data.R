# Prepare_TH_data_for_webpage - EvoSeq gene expression data version from 20210528
...
devseq_genes_input_file <- "TH_genes_inter_norm_tpm_mat_deseq_sample_names.csv"
devseq_genes_input_file_pol <- "TH_genes_intra_norm_tpm_mat_deseq_sample_names.csv"


# Read raw data
devseq_genes_all_samples <- read.table(file=file.path(in_dir, "TH", devseq_genes_input_file), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
devseq_genes_all_samples_pol <- read.table(file=file.path(in_dir, "TH", devseq_genes_input_file_pol), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
