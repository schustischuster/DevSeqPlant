# Prepare_TH_data_for_webpage - EvoSeq transcript expression data version from 20210528
...
devseq_transcripts_input_file <- "TH_transcripts_inter_norm_tpm_mat_deseq_sample_names.csv"
devseq_transcripts_input_file_pol <- "TH_transcripts_intra_norm_tpm_mat_deseq_sample_names.csv"


# Read raw data
devseq_transcripts_all_samples <- read.table(file=file.path(in_dir, "TH", devseq_transcripts_input_file), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
devseq_transcripts_all_samples_pol <- read.table(file=file.path(in_dir, "TH", devseq_transcripts_input_file_pol), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
...
devseq_transcripts_all_samples_pol <- devseq_transcripts_all_samples_pol[c("id", 
	"flowers_mature_pollen_11w_.1.", "flowers_mature_pollen_11w_.2.","flowers_mature_pollen_11w_.3.")]
...
devseq_transcripts_all_samples = devseq_transcripts_all_samples %>% select(
			transcript_id, 
			root_whole_root_5d_.1.:flower_stg12_11w_.3.,
			flower_mid_stg12_equiv_stamens_11w_.1.,
			flower_mid_stg12_equiv_stamens_11w_.2.,
			flower_mid_stg12_equiv_stamens_11w_.3.,
			flowers_mature_pollen_11w_.1.,
			flowers_mature_pollen_11w_.2.,
			flowers_mature_pollen_11w_.3.,
			flower_stg12_equiv_carpels_11w_.1.,
			flower_stg12_equiv_carpels_11w_.2.,
			flower_stg12_equiv_carpels_11w_.3.
			)
...
write.table(devseq_transcripts_all_samples, file=file.path(out_dir, "output", "data_tables", "TH_devseq_transcripts_all_samples.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_transcripts_all_samples_RE, file=file.path(out_dir, "output", "data_tables", "TH_devseq_transcripts_all_samples_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts, file=file.path(out_dir, "output", "data_tables", "TH_devseq_replicate_transcripts.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts_RE, file=file.path(out_dir, "output", "data_tables", "TH_devseq_replicate_transcripts_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
