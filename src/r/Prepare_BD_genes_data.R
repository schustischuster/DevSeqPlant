...
devseq_genes_input_file <- "BD_genes_inter_norm_tpm_mat_deseq_sample_names.csv"
devseq_genes_input_file_pol <- "BD_genes_intra_norm_tpm_mat_deseq_sample_names.csv"


# Read raw data
devseq_genes_all_samples <- read.table(file=file.path(in_dir, "BD", devseq_genes_input_file), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
devseq_genes_all_samples_pol <- read.table(file=file.path(in_dir, "BD", devseq_genes_input_file_pol), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
...
write.table(devseq_genes_all_samples, file=file.path(out_dir, "output", "data_tables", "BD_devseq_genes_all_samples.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_genes_all_samples_RE, file=file.path(out_dir, "output", "data_tables", "BD_devseq_genes_all_samples_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_genes, file=file.path(out_dir, "output", "data_tables", "BD_devseq_replicate_genes.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_genes_RE, file=file.path(out_dir, "output", "data_tables", "BD_devseq_replicate_genes_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
