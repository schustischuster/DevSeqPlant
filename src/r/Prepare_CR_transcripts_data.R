# Prepare_CR_data_for_webpage - EvoSeq transcript expression data version from 20210528
# TPM expression estimates normalized by DESeq2
# This script prepares DevSeq C.rubella expression data input for DevSeqPlant mySQL database
...
devseq_transcripts_input_file <- "CR_transcripts_inter_norm_tpm_mat_deseq_sample_names.csv"
devseq_transcripts_input_file_pol <- "CR_transcripts_intra_norm_tpm_mat_deseq_sample_names.csv"


# Read raw data
devseq_transcripts_all_samples <- read.table(file=file.path(in_dir, "CR", devseq_transcripts_input_file), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
devseq_transcripts_all_samples_pol <- read.table(file=file.path(in_dir, "CR", devseq_transcripts_input_file_pol), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
...
devseq_transcripts_all_samples_pol <- devseq_transcripts_all_samples_pol[c("id", 
	"flowers_mature_pollen_6w.7w.22d_.1.", "flowers_mature_pollen_6w.7w.22d_.2.","flowers_mature_pollen_6w.7w.22d_.3.")]
...
devseq_transcripts_all_samples = devseq_transcripts_all_samples %>% select(
			transcript_id, 
			root_whole_root_4d_.1.:flower_stg12_6w.7w.20d_.3.,
			flower_mid_stg12_stamens_6w.7w.20d_.1.,
			flower_mid_stg12_stamens_6w.7w.20d_.2.,
			flower_mid_stg12_stamens_6w.7w.20d_.3.,
			flowers_mature_pollen_6w.7w.22d_.1.,
			flowers_mature_pollen_6w.7w.22d_.2.,
			flowers_mature_pollen_6w.7w.22d_.3.,
			flower_early_stg12_carpels_6w.7w.21d_.1.,
			flower_early_stg12_carpels_6w.7w.21d_.2.,
			flower_early_stg12_carpels_6w.7w.21d_.3.
			)
...
write.table(devseq_transcripts_all_samples, file=file.path(out_dir, "output", "data_tables", "CR_devseq_transcripts_all_samples.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_transcripts_all_samples_RE, file=file.path(out_dir, "output", "data_tables", "CR_devseq_transcripts_all_samples_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts, file=file.path(out_dir, "output", "data_tables", "CR_devseq_replicate_transcripts.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts_RE, file=file.path(out_dir, "output", "data_tables", "CR_devseq_replicate_transcripts_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
