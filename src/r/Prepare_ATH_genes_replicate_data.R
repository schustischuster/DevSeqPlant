...
replicate_names <- unique(gsub('.{2}$', "", names(devseq_genes_all_samples[4:ncol(devseq_genes_all_samples)])))
...
devseq_replicate_genes = devseq_replicate_genes %>% select(
			gene_id, 
			symbol, 
			root_root_tip_5d, 
...
write.table(devseq_replicate_genes, file=file.path(out_dir, "output", "data_tables", "devseq_replicate_genes.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_genes_RE, file=file.path(out_dir, "output", "data_tables", "devseq_replicate_genes_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
