...
core_ortho_df_RE <- getRE(core_ortho_df)
...
core_ortho_df_RE <- cbind(core_ortho_df_RE[,1:9], round_df(core_ortho_df_RE[,10:ncol(core_ortho_df_RE)], 2))
...
write.table(core_ortho_df_RE, file=file.path(out_dir, "output", "data_tables", "core_ortholog_multi_gene_df_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
