...
# Read raw data
devseq_transcripts_all_samples <- read.table(file=file.path(in_dir, "MT", devseq_transcripts_input_file), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
devseq_transcripts_all_samples_pol <- read.table(file=file.path(in_dir, "MT", devseq_transcripts_input_file_pol), sep=";", dec=".", header=TRUE, stringsAsFactors = FALSE)
...
devseq_transcripts_all_samples_pol <- devseq_transcripts_all_samples_pol[c("id", 
	"flowers_mature_pollen_7w_.1.", "flowers_mature_pollen_7w_.2.","flowers_mature_pollen_7w_.3.")]
...
write.table(devseq_transcripts_all_samples, file=file.path(out_dir, "output", "data_tables", "MT_devseq_transcripts_all_samples.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_transcripts_all_samples_RE, file=file.path(out_dir, "output", "data_tables", "MT_devseq_transcripts_all_samples_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts, file=file.path(out_dir, "output", "data_tables", "MT_devseq_replicate_transcripts.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
write.table(devseq_replicate_transcripts_RE, file=file.path(out_dir, "output", "data_tables", "MT_devseq_replicate_transcripts_RE.csv"), sep=";", dec=".", row.names=FALSE, col.names=TRUE)
